...
class _team_database:
    ...
    def get_teams(self):
        self.db.query('''select t.team_id, t.name as team_name, p.pool_time, t.wins, t.losses, t.ties, l.sport 
    ...
    def get_team(self, team_id):
        lock.acquire()
        self.db.query('''select t.team_id, t.name, t.pool_id,  cast(p.pool_time as char) as pool_time, l.league_id
            from Teams t natural join Pools p natural join Leagues l
        ...
        if type(teams_in_pool) is dict: # only returned one item
            if len(teams_in_pool) == 0:
                return_dict.update({'users': None})
            else:
                return_dict.update({"users": [teams_in_pool["user_id"]]})
        else: # returned multiple items
            return_dict.update({"users": [sql_return["user_id"] for sql_return in teams_in_pool]})
